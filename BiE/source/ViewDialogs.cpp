...
void CpMvDialog::exec(ThumbnailView *thumbView, QString &destDir, bool pasteInCurrDir)
{
	int res = 0;
	QString sourceFile;
	QFileInfo fileInfo;
	...
	if (pasteInCurrDir)
	{
		for (tn = 0; tn < BimData::copyCutFileList.size(); ++tn)
		{
			sourceFile = BimData::copyCutFileList[tn];
			fileInfo = QFileInfo(sourceFile);
			currFile = fileInfo.fileName();
			destFile = destDir + QDir::separator() + currFile;

			opLabel->setText((BimData::copyOp? tr("Copying \"%1\" to \"%2\".") : tr("Moving \"%1\" to \"%2\"."))
											.arg(sourceFile).arg(destFile));
			QApplication::processEvents();

			res = cpMvFile(BimData::copyOp, currFile, sourceFile, destFile, destDir);
			...
			{
				BimData::copyCutFileList[tn] = destFile;
			}
		}
	}
	else
	{
		QList<int> rowList;
		for (tn = BimData::copyCutIdxList.size() - 1; tn >= 0 ; --tn)
		{
			sourceFile = thumbView->thumbViewModel->item(BimData::copyCutIdxList[tn].row())->
			...
			rowList.append(BimData::copyCutIdxList[tn].row());
		}
	...
	nfiles = BimData::copyCutIdxList.size();
	close();	
}
...
void ShortcutsTableView::keyPressEvent(QKeyEvent *e)
{
	...
	QMapIterator<QString, QAction *> it(BimData::actionKeys);
	while (it.hasNext())
	{
		it.next();
	...
	BimData::actionKeys.value(mod->item(row, 2)->text())->setShortcut(QKeySequence(keySeqText));
}
...
void ShortcutsTableView::clearShortcut()
{
	if (selectedEntry.isValid()) {
		...
		BimData::actionKeys.value(mod->item(selectedEntry.row(), 2)->text())->setShortcut(QKeySequence(""));
	}
}
...
SettingsDialog::SettingsDialog(QWidget *parent) : QDialog(parent)
{
    ...
    setWindowIcon(QIcon::fromTheme("preferences-other", QIcon(":/images/bim.png")));
	...
	fitLargeRadios[BimData::zoomOutFlags]->setChecked(true);
	...
	fitSmallRadios[BimData::zoomInFlags]->setChecked(true);
	...
	saveQualitySpin->setValue(BimData::defaultSaveQuality);
	...
	setButtonBgColor(BimData::thumbsBackgroundColor, colThumbButton);
	colThumbButton->setAutoFillBackground(true);
	thumbBgColor = BimData::thumbsBackgroundColor;
	...
	setButtonBgColor(BimData::thumbsTextColor, colThumbTextButton);
	colThumbTextButton->setAutoFillBackground(true);
	thumbTextColor = BimData::thumbsTextColor;
	...
	thumbsBackImageEdit->setText(BimData::thumbsBackImage);
	...
	thumbSpacingSpin->setValue(BimData::thumbSpacing);
	...
	noSmallThumbCb->setChecked(BimData::noEnlargeSmallThumb);
	...
	thumbPagesSpin->setValue(BimData::thumbPagesReadahead);
	...
	enableThumbExifCb->setChecked(BimData::exifThumbRotationEnabled);
	...
	slideDelaySpin->setValue(BimData::slideShowDelay);
	...
	slideRandomCb->setChecked(BimData::slideShowRandom);
	...
	startupDirRadios[BimData::defaultDir] = 
					new QRadioButton(tr("Default, or specified by command line argument"));
	startupDirRadios[BimData::rememberLastDir] = new QRadioButton(tr("Remember last"));
	startupDirRadios[BimData::specifiedDir] = new QRadioButton(tr("Specify:"));
	...
	if (BimData::startupDir == BimData::specifiedDir)
		startupDirRadios[BimData::specifiedDir]->setChecked(true);
	else if (BimData::startupDir == BimData::rememberLastDir)
		startupDirRadios[BimData::rememberLastDir]->setChecked(true);
	else
		startupDirRadios[BimData::defaultDir]->setChecked(true);
	startupDirEdit->setText(BimData::specifiedStartDir);
	...
	{
		it.next();
		keysTable->addRow(it.key(), BimData::actionKeys.value(it.key())->text(), BimData::actionKeys.value(it.key())->shortcut().toString());
	}

	// Mouse settings
	reverseMouseCb = new QCheckBox(tr("Swap mouse left-click and middle-click actions"), this);
	reverseMouseCb->setChecked(BimData::reverseMouseBehavior);

	// Delete confirmation setting
	deleteConfirmCb = new QCheckBox(tr("Delete confirmation"), this);
	deleteConfirmCb->setChecked(BimData::deleteConfirm);
...
void SettingsDialog::saveSettings()
{
	...
	for (i = 0; i < nZoomRadios; ++i) {
		if (fitLargeRadios[i]->isChecked()) {
			BimData::zoomOutFlags = i;
			BimData::appSettings->setValue("zoomOutFlags", (int)BimData::zoomOutFlags);
			break;
		}
	}
	...
	for (i = 0; i < nZoomRadios; ++i) {
		if (fitSmallRadios[i]->isChecked()) {
			BimData::zoomInFlags = i;
			BimData::appSettings->setValue("zoomInFlags", (int)BimData::zoomInFlags);
			break;
		}
	}

	BimData::backgroundColor = bgColor;
	BimData::thumbsBackgroundColor = thumbBgColor;
	BimData::thumbsTextColor = thumbTextColor;
	BimData::thumbsBackImage = thumbsBackImageEdit->text();
	BimData::thumbSpacing = thumbSpacingSpin->value();
	BimData::thumbPagesReadahead = thumbPagesSpin->value();
	BimData::exitInsteadOfClose = exitCliCb->isChecked();
	BimData::wrapImageList = wrapListCb->isChecked();
	BimData::defaultSaveQuality = saveQualitySpin->value();
	BimData::noEnlargeSmallThumb = noSmallThumbCb->isChecked();
	BimData::slideShowDelay = slideDelaySpin->value();
	BimData::slideShowRandom = slideRandomCb->isChecked();
	BimData::enableAnimations = enableAnimCb->isChecked();
	BimData::exifRotationEnabled = enableExifCb->isChecked();
	BimData::exifThumbRotationEnabled = enableThumbExifCb->isChecked();
	BimData::enableImageInfoFS = imageInfoCb->isChecked();
	BimData::reverseMouseBehavior = reverseMouseCb->isChecked();
	BimData::deleteConfirm = deleteConfirmCb->isChecked();

	if (startupDirRadios[0]->isChecked())
		BimData::startupDir = BimData::defaultDir;
	else if (startupDirRadios[1]->isChecked())
		BimData::startupDir = BimData::rememberLastDir;
	else {
		BimData::startupDir = BimData::specifiedDir;
		BimData::specifiedStartDir = startupDirEdit->text();
	}
...
{
	QColor userColor = QColorDialog::getColor(BimData::backgroundColor, this);
...
{
	QColor userColor = QColorDialog::getColor(BimData::thumbsBackgroundColor, this);
	if (userColor.isValid()) {	
...
{
	QColor userColor = QColorDialog::getColor(BimData::thumbsTextColor, this);
	if (userColor.isValid()) {	
...
CropDialog::CropDialog(QWidget *parent, ImageViewer *imageView_) : QDialog(parent)
{
	setWindowTitle(tr("Cropping"));
	setWindowIcon(QIcon(":/images/crop.png"));
	resize(350, 100);
	if (BimData::dialogLastX)
		move(BimData::dialogLastX, BimData::dialogLastY);
...
{
	BimData::cropLeftPercent = leftSpin->value();
	BimData::cropTopPercent = topSpin->value();
	BimData::cropWidthPercent = rightSpin->value();
	BimData::cropHeightPercent = bottomSpin->value();
	imageView->refresh();
}

void CropDialog::ok()
{
	BimData::dialogLastX = pos().x();
	BimData::dialogLastY = pos().y(); 
	accept();
}
...
ResizeDialog::ResizeDialog(QWidget *parent, ImageViewer *imageView_) : QDialog(parent)
{
	setWindowTitle(tr("Scale Image"));
    setWindowIcon(QIcon::fromTheme("transform-scale", QIcon(":/images/bim.png")));
...
void ResizeDialog::ok()
{
	if (newWidth || newHeight) {
		BimData::scaledWidth = newWidth;
		BimData::scaledHeight = newHeight; 
		imageView->refresh();
	}
	accept();
}
...
ColorsDialog::ColorsDialog(QWidget *parent, ImageViewer *imageView_) : QDialog(parent)
{
	...
	colorizeCb->setCheckState(BimData::colorizeEnabled? Qt::Checked : Qt::Unchecked);
	...
	redB->setChecked(BimData::hueRedChannel);
	...
	greenB->setChecked(BimData::hueGreenChannel);
	...
	blueB->setChecked(BimData::hueBlueChannel);
...
void ColorsDialog::applyColors(int)
{
	if (brightSlide->value() >= 0)
		BimData::brightVal = (brightSlide->value() * 500 / 100) + 100;
	else
		BimData::brightVal = brightSlide->value() + 100;

	if (contrastSlide->value() >= 0)
		BimData::contrastVal = (contrastSlide->value() * 79 / 100) + 78;
	else
		BimData::contrastVal = contrastSlide->value() + 79;

	BimData::hueVal = hueSlide->value() * 127 / 100;

	if (saturationSlide->value() >= 0)
		BimData::saturationVal = (saturationSlide->value() * 500 / 100) + 100;
	else
		BimData::saturationVal = saturationSlide->value() + 100;

	if (lightnessSlide->value() >= 0)
		BimData::lightnessVal = (lightnessSlide->value() * 200 / 100) + 100;
	else
		BimData::lightnessVal = lightnessSlide->value() + 100;

	BimData::redVal = redSlide->value();
	BimData::greenVal = greenSlide->value();
	BimData::blueVal = blueSlide->value();
...
{
	BimData::colorizeEnabled = state;
	imageView->refresh();
}

void ColorsDialog::setRedChannel()
{
	BimData::hueRedChannel = redB->isChecked();
	imageView->refresh();
}

void ColorsDialog::setGreenChannel()
{
	BimData::hueGreenChannel = greenB->isChecked();
	imageView->refresh();
}

void ColorsDialog::setBlueChannel()
{
	BimData::hueBlueChannel = blueB->isChecked();
	imageView->refresh();
}
...
AppMgmtDialog::AppMgmtDialog(QWidget *parent) : QDialog(parent)
{
    ...
    setWindowIcon(QIcon::fromTheme("document-properties", QIcon(":/images/bim.png")));
	resize(350, 250);
	...
	QString key, val;
	QMapIterator<QString, QString> it(BimData::externalApps);
	while (it.hasNext())
	{
		it.next();
		key = it.key();
...
void AppMgmtDialog::ok()
{
	...
	BimData::externalApps.clear();
    for (int i = 0; i < row ; ++i)
    {
	    if (!appsTableModel->itemFromIndex(appsTableModel->index(i, 1))->text().isEmpty())
	    {
			BimData::externalApps[appsTableModel->itemFromIndex(appsTableModel->index(i, 0))->text()] =
...
{
	BimData::bookmarkPaths.clear();
    for (int i = 0; i < pathsTableModel->rowCount(); ++i) {
    	BimData::bookmarkPaths.insert
